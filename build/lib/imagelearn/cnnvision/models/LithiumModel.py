...
from imagelearn.cnnvision.HeliumTrainer import HeliumTrainer
...
class LithiumModel(HeliumTrainer):
    def __init__(self, data_set,
                 session=None,
                 num_epochs=None,
                 train_dir='/tmp/beryllium/train'):
        super(LithiumModel, self).__init__(session, train_dir)
        self.num_epochs = num_epochs if num_epochs is not None else 1
        self.data_set = data_set

    def model(self, images, labels=None, is_training=False):
    ...
    def load_batch(self, batch_size=32, is_training=False):
        return self.data_set.load_batch(batch_size=batch_size,
