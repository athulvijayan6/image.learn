...
from image.preprocessing import vgg_preprocessing

class SimpleCNN(Helium):

    def __init__(self, graph= tf.Graph(), session= None, train_dir= '/tmp/simple_cnn/train'):
        super(SimpleCNN, self).__init__(graph, session, train_dir)
    ...
    def evaluate(self, dataset, checkpoint_path= None, batch_size= 32):
        ...
        with self.graph.as_default():
            ...
            if not checkpoint_path: checkpoint_path = tf.train.latest_checkpoint(self.train_dir)
